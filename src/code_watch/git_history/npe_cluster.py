...
1. Load the JSON produced by ``code_watch.git_history.fix_commits.collect_fix_commits``.
...
3. Embed / cluster / label the survivors with the exact machinery from
   ``code_watch.git_history.cluster_fixes`` (sentence-transformers embeddings
...
4. Write/return a JSON report shaped like the ``cluster_fix_messages`` report
   plus NPE-specific fields (``matched_total``, ``excluded_count``, and a
   ``matched_terms_sample`` per cluster).

CLI: ``python -m code_watch.git_history.npe_cluster <commits_json>
...
# Report shaping.
_TERMS_SAMPLE = 5  # max matched terms echoed per cluster
...
def _attach_matched_terms_samples(
    report: dict, commits: list[dict], labels, *, max_terms: int = _TERMS_SAMPLE
) -> None:
    """Add a frequency-ranked ``matched_terms_sample`` to each cluster entry."""
    counted: dict[int, Counter] = {}
    for commit, label in zip(commits, labels):
        cid = int(label)
        if cid >= 0:
            counted.setdefault(cid, Counter()).update(commit.get("matched_terms", []))
    for cluster in report["clusters"]:
        counter = counted.get(cluster["id"], Counter())
        cluster["matched_terms_sample"] = [term for term, _ in counter.most_common(max_terms)]
...
def cluster_npe_commits(
    ...
    *,
    ...
    Returns:
        ...
        * per-cluster ``matched_terms_sample``: most frequent matched terms
          among the cluster's members (max 5, ties broken by first
          occurrence), e.g. ``["NPE", "null (fix context)"]``.
    ...
    if commits:
        ...
        _attach_matched_terms_samples(report, commits, labels)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m code_watch.git_history.npe_cluster",
